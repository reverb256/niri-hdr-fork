...
use smithay::backend::renderer::{Renderer, Bind, DebugFlags, ImportDma, ImportEgl, RendererSuper};
        ...
        for event in scan_result {
            match event {
                ...
                DrmScanEvent::Changed {
                    ...
                    // Re-probe HDR caps: the EDID/modes may have changed (e.g. a TV finishing
                    // its HDR handshake flips edid_pq false->true). Update the output's caps
                    // so output_blend_description() reports PQ/BT.2020 instead of SDR.
                    if let Some((mode, _fallback)) = pick_mode(&connector, None) {
                        if let Ok(surface) = device
                            .drm
